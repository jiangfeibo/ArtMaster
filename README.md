...
[NOTE]本次训练基于Flux.1-dev基础模型，采用AdaLoRA微调方法。数据集由经过裁剪与处理的画家风格样本构成，分辨率为512×512和512×768。训练使用AdamW优化器，学习率设为2e-4，共进行15个epoch，batch size为1。每张图片在训练中被重复学习10次，总训练步数约为6000步。
