...
基础模型：基于开源的FLUX.1-dev模型，FLUX.1-dev在prompt遵从、细节刻画等方面表现优异，接近闭源最先进模型。

Lora微调：Lora是一种高效的模型微调方法，它通过在大模型中的部分层中插入低秩矩阵来学习新特征，而无需改动原始权重，从而大幅度降低训练参数量与显存消耗。
